...
void *World::GetComponent(EntityID component, std::type_index ComponentType) {
  ...
  if(it != m_Components.end()) { return it->second->Get(component); }
  return nullptr;
}
...
void World::Kill(EntityRef entity) { m_Entities.erase(entity.ID()); }
...
bool World::operator==(const World &rhs) { return (this == &rhs); }
