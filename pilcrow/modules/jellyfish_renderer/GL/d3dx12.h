...
struct CD3DX12_BOX : public D3D12_BOX {
  ...
  explicit CD3DX12_BOX(
    LONG Left, LONG Top, LONG Front, LONG Right, LONG Bottom, LONG Back) {
    left   = static_cast<UINT>(Left);
    top    = static_cast<UINT>(Top);
...
struct CD3DX12_ROOT_CONSTANTS : public D3D12_ROOT_CONSTANTS {
  ...
  inline void
  Init(UINT num32BitValues, UINT shaderRegister, UINT registerSpace = 0) {
...
struct CD3DX12_ROOT_PARAMETER : public D3D12_ROOT_PARAMETER {
  ...
  static inline void InitAsConstants(_Out_ D3D12_ROOT_PARAMETER &rootParam,
    ...
    rootParam.ShaderVisibility = visibility;
    CD3DX12_ROOT_CONSTANTS::Init(rootParam.Constants,
                                 num32BitValues,
                                 shaderRegister,
                                 registerSpace);
  }
  ...
  InitAsConstantBufferView(_Out_ D3D12_ROOT_PARAMETER &rootParam,
    ...
    rootParam.ShaderVisibility = visibility;
    CD3DX12_ROOT_DESCRIPTOR::Init(rootParam.Descriptor,
                                  shaderRegister,
                                  registerSpace);
  }
...
struct CD3DX12_ROOT_SIGNATURE_DESC : public D3D12_ROOT_SIGNATURE_DESC {
  ...
  static inline void
  Init(_Out_ D3D12_ROOT_SIGNATURE_DESC &                         desc,
       UINT                                                      numParameters,
       _In_reads_opt_(numParameters) const D3D12_ROOT_PARAMETER *_pParameters,
       UINT numStaticSamplers = 0,
       _In_reads_opt_(numStaticSamplers)
...
struct CD3DX12_ROOT_PARAMETER1 : public D3D12_ROOT_PARAMETER1 {
  ...
  InitAsDescriptorTable(_Out_ D3D12_ROOT_PARAMETER1 &rootParam,
    ...
    rootParam.ParameterType    = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE;
    rootParam.ShaderVisibility = visibility;
    CD3DX12_ROOT_DESCRIPTOR_TABLE1::Init(rootParam.DescriptorTable,
                                         numDescriptorRanges,
                                         pDescriptorRanges);
  }
  ...
  static inline void InitAsConstantBufferView(
    ...
    rootParam.ParameterType    = D3D12_ROOT_PARAMETER_TYPE_CBV;
    rootParam.ShaderVisibility = visibility;
    CD3DX12_ROOT_DESCRIPTOR1::Init(rootParam.Descriptor,
                                   shaderRegister,
                                   registerSpace,
                                   flags);
  }
  ...
  inline void InitAsUnorderedAccessView(UINT shaderRegister,
                                        ...
                                        D3D12_SHADER_VISIBILITY visibility
                                        = D3D12_SHADER_VISIBILITY_ALL) {
    InitAsUnorderedAccessView(*this,
                              shaderRegister,
                              registerSpace,
                              flags,
                              visibility);
...
struct CD3DX12_RESOURCE_DESC : public D3D12_RESOURCE_DESC {
  ...
  inline UINT CalcSubresource(UINT MipSlice, UINT ArraySlice, UINT PlaneSlice) {
    return D3D12CalcSubresource(MipSlice,
                                ArraySlice,
                                PlaneSlice,
                                MipLevels,
                                ArraySize());
...
inline UINT64 UpdateSubresources(
  ...
  if(DestinationDesc.Dimension == D3D12_RESOURCE_DIMENSION_BUFFER) {
    pCmdList->CopyBufferRegion(pDestinationResource,
                               0,
                               pIntermediate,
                               pLayouts[0].Offset,
...
private:
  ...
  typedef struct SUBOBJECT_WRAPPER : public D3D12_STATE_SUBOBJECT {
    D3D12_STATE_SUBOBJECT *pSubobjectArrayLocation;  // new location when
                                                     // flattened into array for
                                                     // repointing pointers in
                                                     // subobjects
  } SUBOBJECT_WRAPPER;
  D3D12_STATE_OBJECT_DESC      m_Desc;
  std::list<SUBOBJECT_WRAPPER> m_SubobjectList;  // Pointers to list nodes
                                                 // handed out so these can be
                                                 // edited live
  std::vector<D3D12_STATE_SUBOBJECT> m_SubobjectArray;  // Built at the end,
