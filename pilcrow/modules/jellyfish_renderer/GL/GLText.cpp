...
GLText::GLText(const std::string &shader, const std::string &font, int size)
  ...
  for(GLubyte c{' '}; c <= '~'; ++c) {
    ...
    glTexImage2D(GL_TEXTURE_2D, 0, GL_RED, m_Font->glyph->bitmap.width,
...
void GLText::Render(const std::string &text, glm::vec2 position, glm::mat4 proj,
  ...
  for(char c : text) {
    ...
    glBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(vertices),
                    vertices);  // Be sure to use glBufferSubData and not
                                // glBufferData
    ...
    position.x += (ch.Advance >> 6) * scale;  // Bitshift by 6 to get value in
                                              // pixels (2^6 = 64 (divide amount
                                              // of 1/64th pixels by 64 to get
                                              // amount of pixels))
  }
