...
void DeviceResources::CreateDeviceResources() {
  ...
  // active device.
  {
    ...
    if(SUCCEEDED(
         DXGIGetDebugInterface1(0,
                                IID_PPV_ARGS(dxgiInfoQueue.GetAddressOf())))) {
  ...
  // Determines whether tearing support is available for fullscreen borderless
  // windows.
  ...
  ThrowIfFailed(
    m_d3dDevice->CreateCommandQueue(&queueDesc,
  ...
  ThrowIfFailed(
    m_d3dDevice
      ->CreateDescriptorHeap(&rtvDescriptorHeapDesc,
                             IID_PPV_ARGS(
                               m_rtvDescriptorHeap.ReleaseAndGetAddressOf())));
  ...
  if(m_depthBufferFormat != DXGI_FORMAT_UNKNOWN) {
    ...
    ThrowIfFailed(
      m_d3dDevice->CreateDescriptorHeap(&dsvDescriptorHeapDesc,
                                        IID_PPV_ARGS(
                                          m_dsvDescriptorHeap
                                            .ReleaseAndGetAddressOf())));
  ...
  for(UINT n = 0; n < m_backBufferCount; n++) {
    ThrowIfFailed(
      m_d3dDevice->CreateCommandAllocator(D3D12_COMMAND_LIST_TYPE_DIRECT,
                                          IID_PPV_ARGS(
                                            m_commandAllocators[n]
                                              .ReleaseAndGetAddressOf())));
  }
  ...
  if(!m_fenceEvent.IsValid()) { throw std::exception("CreateEvent"); }
}
...
void DeviceResources::CreateWindowSizeDependentResources() {
  ...
  UINT backBufferWidth
    = std::max<UINT>(m_outputSize.right - m_outputSize.left, 1);
  ...
  // Obtain the back buffers for this window which will be the final render
  // targets and create render target views for each of them.
  for(UINT n = 0; n < m_backBufferCount; n++) {
    ThrowIfFailed(
      m_swapChain->GetBuffer(n,
                             IID_PPV_ARGS(m_renderTargets[n].GetAddressOf())));
    ...
    rtvDescriptor(m_rtvDescriptorHeap->GetCPUDescriptorHandleForHeapStart(),
                  n,
                  m_rtvDescriptorSize);
    m_d3dDevice->CreateRenderTargetView(m_renderTargets[n].Get(),
                                        &rtvDesc,
                                        rtvDescriptor);
  }
  ...
  if(m_depthBufferFormat != DXGI_FORMAT_UNKNOWN) {
    // Allocate a 2-D surface as the depth/stencil buffer and create a
    // depth/stencil view on this surface.
    ...
    D3D12_RESOURCE_DESC depthStencilDesc
      = CD3DX12_RESOURCE_DESC::Tex2D(m_depthBufferFormat,
                                     backBufferWidth,
                                     backBufferHeight,
                                     1,  // This depth stencil view has only one
                                         // texture.
    ...
    ThrowIfFailed(
      m_d3dDevice
        ->CreateCommittedResource(&depthHeapProperties,
    ...
    m_d3dDevice
      ->CreateDepthStencilView(m_depthStencil.Get(),
                               &dsvDesc,
                               m_dsvDescriptorHeap
                                 ->GetCPUDescriptorHandleForHeapStart());
  }

  // Set the 3D rendering viewport and scissor rectangle to target the entire
  // window.
...
bool DeviceResources::WindowSizeChanged(int width, int height) {
  ...
  newRc.right            = width;
  newRc.bottom           = height;
  if(newRc.left == m_outputSize.left && newRc.top == m_outputSize.top
...
void DeviceResources::Prepare(D3D12_RESOURCE_STATES beforeState) {
  ...
  ThrowIfFailed(
    m_commandList->Reset(m_commandAllocators[m_backBufferIndex].Get(),
                         nullptr));

  // Reset resources.
  m_frameResources[m_backBufferIndex].clear();

  if(beforeState != D3D12_RESOURCE_STATE_RENDER_TARGET) {
    // Transition the render target into the correct state to allow for drawing
    // into it.
    D3D12_RESOURCE_BARRIER barrier = CD3DX12_RESOURCE_BARRIER::
      Transition(m_renderTargets[m_backBufferIndex].Get(),
                 beforeState,
...
void DeviceResources::Present(D3D12_RESOURCE_STATES beforeState) {
  if(beforeState != D3D12_RESOURCE_STATE_PRESENT) {
    // Transition the render target to the state that allows it to be presented
    // to the display.
    D3D12_RESOURCE_BARRIER barrier
      = CD3DX12_RESOURCE_BARRIER::Transition(m_renderTargets[m_backBufferIndex]
                                               .Get(),
                                             beforeState,
  ...
  HRESULT hr;
  if(m_options & c_AllowTearing) {
    // Recommended to always use tearing if supported when using a sync interval
    // of 0. Note this will fail if in true 'fullscreen' mode.
  ...
  if(hr == DXGI_ERROR_DEVICE_REMOVED || hr == DXGI_ERROR_DEVICE_RESET) {
    ...
    if(!m_dxgiFactory->IsCurrent()) {
      // Output information is cached on the DXGI Factory. If it is stale we
...
void DeviceResources::MoveToNextFrame() {
  ...
  if(m_fence->GetCompletedValue() < m_fenceValues[m_backBufferIndex]) {
    ThrowIfFailed(
      m_fence->SetEventOnCompletion(m_fenceValues[m_backBufferIndex],
                                    m_fenceEvent.Get()));
    WaitForSingleObjectEx(m_fenceEvent.Get(), INFINITE, FALSE);
  }
...
// This method acquires the first available hardware adapter that supports
// Direct3D 12. If no such adapter can be found, try WARP. Otherwise throw an
// exception.
void DeviceResources::GetAdapter(IDXGIAdapter1 **ppAdapter) {
  *ppAdapter = nullptr;
  ...
  for(UINT adapterIndex = 0;
      ...
      != m_dxgiFactory->EnumAdapters1(adapterIndex,
  ...
  if(!adapter) {
    // Try WARP12 instead
    if(FAILED(m_dxgiFactory->EnumWarpAdapter(
  ...
  if(!adapter) { throw std::exception("No Direct3D 12 device found"); }
